...
T **Allocate_2D( T **p2D, int nx, int ny )
{
	...
	if( p == 0 )
		return 0 ;
	p[0] = new ( std::nothrow ) T[ long long int(nx) * long long int(ny) ] ;
	if( p[0] == 0 )
	{
	...
	if( ( typeid(p[0][0]) == typeid( int ) ) || ( typeid(p[0][0]) == typeid( float ) ) ||
		...
		memset( p[0] , 0 , sizeof( T ) * long long int(nx) * long long int(ny) ) ;
...
T ***Allocate_3D( T ***p3D, int nx, int ny , int nz )
{
	...
	p[0][0] = 0 ;
	p[0][0] = new ( std::nothrow ) T[long long int(nx) * long long int(ny) * long long int(nz) ] ;
	...
	for( i = 0 ; i < nx ; i++ )
		for( j = 0 ; j < ny ; j++ )
			p[i][j] = p[0][0] + long long int( i * ny + j ) * nz ;
	...
	{
		long long int size = long long int(nx) * long long int(ny) * long long int(nz) * sizeof( T ) ;
		memset( p[0][0] , 0 , size ) ;
	}
